...
export default function ShelfContent({ initialBooks }: { initialBooks: Book[] }) {
  ...
  return (
    <div className="space-y-8">
      <div className="border-b border-stone-200 pb-8">
        ...
        <p className="text-stone-500">{initialBooks.length} books in your collection</p>
      </div>
        ...
        {FILTERS.map(({ value, label }) => (
          ...
          >
            ...
            <span className="ml-2 opacity-60">
              {value === 'all' ? initialBooks.length : initialBooks.filter((b) => b.status === value).length}
            </span>
          </button>
        ))}
      </div>
        ...
        <div className="text-center py-16 text-stone-400">
          ...
          <p className="text-sm">
            {query
              ...
              : <Link href="/add" className="text-indigo-700 hover:underline">Add your first book</Link>
            }
          </p>
        </div>
      ) : (
